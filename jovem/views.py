...
def indicado_update(request, id):
    ...
    if form.is_valid():
        ...
        if str(form.cleaned_data['status']) == 'Fechado':
            ...
            send_email(request, body.format(indicados.resp_indicacao.nome, indicados.nome), indicados.resp_indicacao.email1)
        ...
        if str(form.cleaned_data['status']) == 'Declinado':
            ...
            send_email(request, body.format(indicados.resp_indicacao.nome, indicados.nome), indicados.resp_indicacao.email1)
            body = email_indicado.texto
            send_email(request, body.format(indicados.nome, indicados.resp_indicacao.nome), indicados.email)
